	...
	public static BobObject eval( BobObject<?> expr, Environment environment ) throws BobException {
		...
		if (expr instanceof Atom ) {
			...
			//if we have nested expression, grab it.
				...
				if ( ((BobList) returned).size() > 0 && ((BobList) returned).car() instanceof BobFunction ) {
					returned = Builtin.lambdaInvoke( (BobFunction) ((BobList) returned).car(), ((BobList) returned).cdr(), environment);
				}
			}
